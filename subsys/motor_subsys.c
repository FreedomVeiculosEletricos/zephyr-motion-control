...
int motor_group_set_speed(struct motor_group *group, const float *speeds)
{
	...
	if ((group == NULL) || (speeds == NULL) || (group->count == 0U)) {
		return -EINVAL;
	}
	...
	for (uint8_t i = 0; i < group->count; i++) {
		r = motor_set_speed(group->members[i], speeds[i]);
		if ((r != 0) && (ret == 0)) {
			ret = r;
		}
	}
...
int motor_group_set_position(struct motor_group *group, const float *angles)
{
	...
	if ((group == NULL) || (angles == NULL) || (group->count == 0U)) {
		return -EINVAL;
	}
	...
	for (uint8_t i = 0; i < group->count; i++) {
		r = motor_set_position(group->members[i], angles[i]);
		if ((r != 0) && (ret == 0)) {
			ret = r;
		}
	}
