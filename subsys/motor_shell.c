...
static void print_motor_status(const struct shell *sh, struct motor_ctrl *m)
{
	...
	if (r == 0) {
		r = motor_algo_dc_current_get_state(m, &dc_state);
		if (r == 0) {
			shell_print(sh, "  DC state: i_ref=%f i_meas=%f err=%f duty=%f",
				    (double)dc_state.i_ref_a, (double)dc_state.i_meas_a,
				    (double)dc_state.error_a, (double)dc_state.duty);
		}
