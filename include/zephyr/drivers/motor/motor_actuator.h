 ...
 * The controller layer writes normalised per-phase duty cycles here
 * and never touches PWM/GPIO peripherals directly.
 *
 ...
 *
 * Duty array length must match @c n_phases in @ref motor_stage_config.
 */
...
/** Max elements in @ref motor_actuator_ops.set_duty and @ref motor_block_out.duty. */
#define MOTOR_ACTUATOR_DUTY_MAX 6

/* ------------------------------------------------------------------ */
/* Actuator callback (PWM period ISR)                                  */
/* ------------------------------------------------------------------ */

/**
 * @brief Callback invoked from power-stage timing interrupts.
 *
 * The fast callback runs once per PWM period. The optional slow callback runs
 * from the actuator-owned slow timer alarm, phase-locked to the PWM ISR.
 */
typedef void (*motor_actuator_callback_t)(const struct device *dev, void *user_data);
...
struct motor_actuator_ops {
	 ...
	 *
	 * Starts PWM generation. Hardware setup (timers, GPIO, gate driver)
	 * is done in the driver instance init, not via this vtable.
	 *
	...
	/**
	 * @brief Apply per-phase duty cycles (ISR-safe).
	 *
	 * Array length must match @c n_phases in the stage config.
	...
	/**
	 * @brief Register fast PWM and slow timer callbacks.
	 *
	 * @param dev        Power stage device.
	 * @param fast_cb    Called from the backend PWM ISR each period (may be NULL).
	 * @param fast_user_data Argument passed to @p fast_cb.
	 * @param slow_cb    Called from the backend slow timer ISR (may be NULL).
	 * @param slow_user_data Argument passed to @p slow_cb.
	 * @retval 0 on success, negative errno on failure.
	 */
	int (*set_callback)(const struct device *dev, motor_actuator_callback_t fast_cb,
...
static inline int motor_actuator_set_callback(const struct device *dev,
					      ...
					      void *slow_user_data)
{
	...
	return ops->set_callback(dev, fast_cb, fast_user_data, slow_cb, slow_user_data);
}
...
static inline const struct motor_stage_config *motor_actuator_get_config(const struct device *dev)
{
	const struct motor_actuator_ops *ops = dev->api;

	return ops->get_config(dev);
}
