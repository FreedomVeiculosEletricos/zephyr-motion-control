 ...
 *      by DT node label or index; no manual wiring.
 *
 ...
 *
 *   5. Fault propagation policy — one member entering FAULT can
 *      trigger group_estop() on all siblings within one supervision
 *      tick (configurable via Kconfig).
 *
 *   6. Shared resource arbitration — if two motor instances share
 *      an ADC or timer peripheral, the subsystem serialises their
 *      update() calls using a per-resource spinlock.
 *
 *   7. Per-algorithm tuning and commands — predefined pipelines
 *      (dc-current, future FOC, …)
 ...
 *
 *       motor_algo_dc_current_set_current(m[0], i_ref0); // algorithm-specific command
 *   }
 */
...
struct motor_subsys_entry {
	...
	/** Algorithm root storage; same as @c motor_ctrl::pipeline_ctx. */
	void *pipeline_ctx;
 ...
 */
#define MOTOR_SUBSYS_DEFINE(_name, _ctrl, _sensor, _actuator, _pipeline, _pipeline_ctx, _label)      \
...
/**
 * @brief Bootstrap all motors declared in Devicetree or with @c MOTOR_SUBSYS_DEFINE.
 *
