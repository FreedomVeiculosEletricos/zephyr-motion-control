 ...
 * @{
 *
 * Use this for generic commands and state (enable, drive mode, faults).
 ...
 *
 * Instances declared in Devicetree are registered by the motor subsystem;
 * sensor, actuator, pipeline, and algorithm data come from Devicetree and static
 * initialisers. There is no
 * application-level “init” beyond Zephyr
 ...
 *   RUN   : motor_set_drive_mode(), motor_disable(), motor_estop()
