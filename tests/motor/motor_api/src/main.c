...
{
	zassert_equal(motor_ctrl_init(&ctrl, NULL, NULL, &motor_algo_dc_current, &algo_data,
...
ZTEST(motor_api_suite, test_motor_ctrl_init_rejects_null_actuator)
{
	zassert_equal(motor_ctrl_init(&ctrl, (const struct device *)&ctrl, NULL,
				      &motor_algo_dc_current, &algo_data, &motor_api_valid_params),
		      -EINVAL);
}
...
{
	zassert_equal(motor_ctrl_init(&ctrl, (const struct device *)&ctrl,
				      (const struct device *)&ctrl, &motor_algo_dc_current, NULL,
				      &motor_api_valid_params),
		      -EINVAL);
}
...
{
	zassert_equal(motor_ctrl_init(&ctrl, (const struct device *)&ctrl,
				      (const struct device *)&ctrl, &motor_algo_dc_current, &algo_data,
				      NULL),
		      -EINVAL);
}
...
{
	zassert_is_null(motor_init(NULL, NULL, NULL, &motor_algo_dc_current, &algo_data,
				   &motor_api_valid_params));
}
